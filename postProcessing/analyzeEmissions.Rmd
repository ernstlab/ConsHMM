...
Add key enrichments.
```{r}
enrichmentsData = read_tsv(paste0("../enrichments/", genomeName, "_", numStates, "_states/", genomeName, ".txt"), n_max=numStates)
enrichments <- enrichmentsData %>%
    select(-one_of('state (User order)', 'Genome %'))
colnames(enrichments) <- enframe(colnames(enrichments)) %>% 
    separate(value, into=c("title"), by='.') %>% 
    select(title) %>%
    deframe
norm = function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}
enrichments = apply(enrichments, 2, norm)

colFunc = colorRampPalette(c("white", "red"))
pdf(paste0("../enrichmentHeatmaps/", genomeName, "_enrichments_", numStates, "_states.pdf"), height=12, width = 6)
pheatmap(enrichments, color = colFunc(10), cluster_rows = FALSE, cluster_cols = FALSE, cellwidth = 10, cellheight = 9, fontsize_col = 7, display_numbers = FALSE, fontsize_number = 3, main1="", main2="", labels_row = c(1:numStates))
dev.off()
```
